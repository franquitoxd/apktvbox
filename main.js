...
function renderApps(list) {
  ...
  // Aca agregamos el index para saber que numero le toca a cada app
  list.forEach((a, index) => {
    const num = index + 1;
    const div = document.createElement('div');
    div.className = 'app-box select-fx';
    div.tabIndex = 0;
    div.dataset.num = num; // Guardamos el numero en el div para buscarlo despues
    div.innerHTML = `
...
// --- NAV TECLADO MEJORADA Y NUMEROS ---
document.addEventListener('keydown', e => {
  ...
  if (/^[0-9]$/.test(e.key)) {
    ...
    typeTimer = setTimeout(() => {
      ...
      appBoxes.forEach(box => {
        if(parseInt(box.dataset.num) === numToFind) {
          box.click();
          found = true;
        }
      });
      
      // Limpiamos todo despues de buscar
      typedNum = "";
      overlay.classList.add('hidden');
    }, 1200); // Tarda 1.2 segundos en desaparecer si no tocas otro numero
    
    return; // Frenamos aca para que no haga la movida de las flechas
  }
  
  // LOGICA DE FLECHAS (Funciona igual adentro del modal de detalles)
  if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.key)) {
    ...
    focusables.forEach(el => {
      ...
      // Un truquito por si estas en un modal y hay pocas opciones (ej: el boton VOLVER y DESCARGAR)
